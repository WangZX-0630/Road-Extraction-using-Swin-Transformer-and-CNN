        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = mmcv.imrescale(
                    results[key], self.scale, interpolation='nearest')
            else:
...
class Pad(object):
    ...
    def _pad_img(self, results):
        ...
        self.pad_shape = padded_img.shape
...
class Rerange(object):
    ...
    def __init__(self, min_value=0, max_value=255, for_seg_field=True):
        assert isinstance(min_value, float) or isinstance(min_value, int)
        assert isinstance(max_value, float) or isinstance(max_value, int)
        assert min_value < max_value
        self.min_value = min_value
        self.max_value = max_value
        self.for_seg_field = for_seg_field

    def _rerange_img(self, img):
    ...
    def __call__(self, results):
        ...
        """
        if self.for_seg_field:
            for key in results.get('seg_fields', []):
                results[key] = self._rerange_img(results[key])

        else:
            results['img'] = self._rerange_img(results['img'])
...
class RandomGamma(object):
    ...
    def __init__(self, gamma=(60.0, 120.0)):
        self.gamma = np.random.uniform(min(gamma), max(gamma))
...
class ToTensor(object):
    ...
    def __call__(self, results):
        ...
        image = np.array(results['img']).astype(np.float32).transpose((2, 0, 1)) 
        results['img'] = torch.from_numpy(image).float()
        for key in results.get('seg_fields', []):
            if key != 'label':
                seg_mask = np.array(results[key]).astype(np.float32).transpose((2, 0, 1))
                results[key] = torch.from_numpy(seg_mask).float()
            else:
                seg_mask = np.array(results[key]).astype(np.float32)
                results[key] = torch.from_numpy(seg_mask).float()
        del results['seg_fields']
        return results
