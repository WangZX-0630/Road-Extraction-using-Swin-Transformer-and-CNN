...
class Segmentation(Dataset):
    ...
    def __init__(self,
        ...
        self._val_img_dir = os.path.join(self._base_dir, 'img_dir/val/')
        self._val_cat_dir = os.path.join(self._base_dir, 'ann_dir/val/')
        ...
        for splt in self.split:
            ...
            if splt == 'train':
                for i, line in enumerate(os.listdir(self._image_dir)):
                ...
                for i, line in enumerate(os.listdir(self._val_img_dir)):
                    _image = os.path.join(self._val_img_dir, line)
                    _cat = os.path.join(self._val_cat_dir, line)
                    self.im_ids.append(line.split('.')[0])
                    self.images.append(_image)
                    self.categories.append(_cat)
    ...
    def transform_tr(self, sample):
        ...
        img_norm_cfg = dict(
            mean=[76.34659911, 78.29393622, 71.42089849], std=[34.34968696, 35.78564323, 40.03962246], to_rgb=True)
        composed_transforms = transforms.Compose([
            mmtf.Resize(img_scale=(self.args.base_size, self.args.base_size), ratio_range=(1.0, 1.2)),
