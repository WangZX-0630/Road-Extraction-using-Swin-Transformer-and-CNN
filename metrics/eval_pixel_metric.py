...
parser.add_argument('--gt_dir', type=str, default="C:/Users/Lenovo/PycharmProjects/VecRoad-master/data/input/test_gt_mask")
parser.add_argument('--pred_dir', type=str, default="C:/Users/Lenovo/PycharmProjects/VecRoad-master/data/input/test_proposal_mask")
...
result_fns = sorted(glob.glob(os.path.join(result_dir, '*.png')))
...
def get_quality(completeness, correctness):
    if completeness == correctness == 0:
        return 0
    return completeness * correctness / (completeness + correctness - completeness * correctness)
...
if __name__ == '__main__':
    ...
    print("precision: {:.2f} Recall: {:.2f} F1: {:.2f} Quality: {:.2f}".format(
        breakeven_pt[0] * 100, breakeven_pt[1] * 100, get_f1(*breakeven_pt) * 100, get_quality(*breakeven_pt) * 100))
    print("max P-F1: {:.2f}".format(np.max(F1) * 100))

    end_time = time.perf_counter()
    print('cost time: %f s' % (end_time - start_time))

#  num workers = 4  16384*16: 30.6s
